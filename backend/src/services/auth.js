...
const verifyProfPassword = (req, res) => {
  argon2
    .verify(req.professor.password, req.body.password, hashingOptions)
    .then((isVerified) => {
      if (isVerified) {
        const token = jwt.sign({ sub: req.professor.id }, JWT_SECRET, {
          algorithm: "HS512",
          expiresIn: JWT_TIMING,
        });
        delete req.professor.password;
        res.send({ token, professor: req.professor });
      } else res.sendStatus(401);
    })
    .catch((err) => {
      // do something with err
